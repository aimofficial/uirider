...
import com.newtaxiprime.user.databinding.AppActivityPaymentAmountPageBinding
...
class PaymentAmountPage : CommonActivity(), ServiceListener, PaymentMethodNonceCreatedListener {
    ...
    lateinit var binding: AppActivityPaymentAmountPageBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        commonMethods.setheaderText(
            resources.getString(R.string.paymentdetails),
        ...
        binding.rateSubmit.setOnClickListener{
            rateSubmit()
        }
        ...
        binding.paymentmethodChange.setOnClickListener{
            gotoPaymentpage()
        }
        recyclerView.layoutManager = layoutManager


        rate_submit = binding.rateSubmit

        wallet_img = binding.walletImg

        paymentmethod_img = binding.paymentmethodImg
        paymentmethod_type = binding.paymentmethodType

        recyclerView = binding.rvPrice

        paymentMethodView = binding.paymentmethod
    ...
    fun loadData() {
        ...
        if (payable_amt <= 0) {
            ...
            rate_submit.background = ContextCompat.getDrawable(
                this,
            ...
            rate_submit.text =
                resources.getString(R.string.proceed) // if payable amount is 0 then directly we can proceed to next step
        } else {
            if (payment_method.contains(
    ...
    fun changeCurrencyPayment(paypalamount: String) {
        commonMethods.showProgressDialog(this)
        apiService.paypalCurrency(
            sessionManager.accessToken.toString(),
            sessionManager.currencyCode.toString(),
            paypalamount
        ).enqueue(RequestCallback(REQ_PAYPAL_CURRENCY, this))
    ...
    fun afterPayment(payKey: String) {
        commonMethods.showProgressDialog(this)
        apiService.proceedAfterPayment(getParamsForPaypalAndStripe(payKey))
            .enqueue(RequestCallback(REQ_AFTER_PAY, this))
    }
 ...
 */
        ...
        if (isInternetAvailable && !isBrainTree) {
            ...
            AddFirebaseDatabase().updatePaymentChangedNode(
                walletType,
