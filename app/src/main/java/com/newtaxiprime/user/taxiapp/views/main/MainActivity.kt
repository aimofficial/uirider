...
import com.newtaxiprime.user.databinding.AppActivityMainBinding
import com.newtaxiprime.user.databinding.AppBarHomeToolbarBinding
import com.newtaxiprime.user.databinding.AppContentMainBinding
...
class MainActivity : CommonActivity(), SeatsListAdapter.OnClickListener, OnMapReadyCallback,
    ...
    LocationListener, ServiceListener,
    ...
    private lateinit var appMainBinding: AppContentMainBinding
    private lateinit var appBarHomeToolBarBinding: AppBarHomeToolbarBinding


    private lateinit var layout_request_seat: View
...
//    }
                    ...
                    if (sessionManager.paymentMethodkey!!.isEmpty() || (sessionManager.paymentMethodkey!!.contains(
                        ...
                        if (isPeakPriceAppliedForClickedCar == CommonKeys.YES) {
                            ...
                            if (!isPooled) {
                                ...
                                binding.appBarMain.headerMain.rvSeatlist
                                appMainBinding.rvSeatlist.adapter = adapter
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        appMainBinding = binding.appBarMain.headerMain
        appBarHomeToolBarBinding = binding.appBarMain.commonHeader

        ButterKnife.bind(this)
        AppController.appComponent.inject(this)

        rideicon = appMainBinding.rideicon
        layout_request_seat = binding.appBarMain.headerMain.layoutRequestSeat


        //layout_request_seat =

        tvWaititingChargeFeeForAcceptedCar =
            appMainBinding.tvWaitingChargeFeeForAcceptedCar

        whereto_and_schedule = appMainBinding.wheretoAndSchedule

        rltChangeLocation = appBarHomeToolBarBinding.rltChangeLocation

        rltWelcome = appBarHomeToolBarBinding.rltWelcome

        tvChangeLocation = appBarHomeToolBarBinding.tvChangeLocation
        ...
        whereto = appMainBinding.whereto    // Where to search page button

        appVersion = binding.tvAppVersion

        no_car_txt = appMainBinding.noCarTxt

        listView = appMainBinding.carDetailsList   // Search car list

        paymentmethod = appMainBinding.paymentmethod       // Payment method selection layout

        paymentmethod_img =
            appMainBinding.paymentmethodImg    // Cash or paypal iamge show for payment method

        wallet_img = appMainBinding.walletImg     // If wallet choose show in image

        sos = appBarHomeToolBarBinding.sos

        paymentmethod_type =
            appMainBinding.paymentmethodType// Payment method type show in text view

        pickup_location = appMainBinding.pickupLocation

        driver_name = appMainBinding.driverName

        driver_car_name = appMainBinding.driverCarName

        driver_rating = appMainBinding.driverRating

        driver_car_number = appMainBinding.driverCarNumber

        tvDriverOTP = appMainBinding.driverOtp

        driver_image = appMainBinding.profileImage1

        arriveTimeImage = appMainBinding.rltNavi

        startChat = appMainBinding.fabStartChat

        ivFilter = appBarHomeToolBarBinding.ivFilter

        tvWelcomeText = appBarHomeToolBarBinding.tvWelcomeText

        meetlayout = appMainBinding.meetlayout       // Show address after trip start

        bottomlayout = appMainBinding.bottomlayout//Search car bottom layout

        bottomlayout1 = appMainBinding.bottomlayout1        //Search car bottom layout

        content_main = appMainBinding.contentMain


        no_car = appMainBinding.noCar

        loading_car = appMainBinding.loadingCar

        requestubers = appMainBinding.requestubers

        edit_map = appMainBinding.editMap

        scheduleride = appMainBinding.scheduleride      // Schedule ride button

        requestuber = appMainBinding.requestuber      // Reqest button

        tvWaitingChargeInfo = appMainBinding.tvWaitingChargeFee      // waiting charge

        textView1 = appMainBinding.textview1

        request_view = appMainBinding.activityFareEstimation//Fare estimate

        show_seat_availability = appMainBinding.layoutRequestSeat

        rltContactAdmin = appMainBinding.rltContactAdmin

        tvEta = appMainBinding.tvEta

        poolRate = appMainBinding.poolRate

        binding.drivewithuber.setOnClickListener {
            drivewithuber()
        }

        rltContactAdmin.setOnClickListener {
            setRltContactAdmin()
        }

        appMainBinding.fabStartChat.setOnClickListener {
            startChatActivity()
        }
        ...
        appMainBinding.backPool.setOnClickListener {
            backPool()
        }
        ...
        scheduleride.setOnClickListener {
            schedulerideSearchCar()
        }

        appMainBinding.confirmSeat.setOnClickListener {
            PoolRequest()
        }

        requestuber.setOnClickListener {
            requestuber()
        }
        ...
        appMainBinding.ride.setOnClickListener {
            bookride()
        }

        appMainBinding.bike.setOnClickListener {
            bike()
        }
        binding.drivewithuber.setOnClickListener {
            drivewithuber()
        ...
        rideicon.setOnClickListener {
            ride()
        }

        appMainBinding.reserve.setOnClickListener {
            reserve()
        }

        bottomlayout1.setOnClickListener {
            bottomlayout()
        }

        appMainBinding.paymentmethodChange.setOnClickListener {
            gotoPaymentpage()
        }
        ...
        //  layout_request_seat = binding.drawerLayout.
                ...
                override fun onResponse(
                    ...
                    if (response.isSuccessful) {
                        ...
                        for (i in 0 until bannersArray.length()) {
                            ...
                            // https://new.newtaxi.co/images/banner/category-image1687646558.jpg
                            val imageUrl =
                                "${getString(R.string.image_url)}banner/" + bannerObject.getString("imageurl")
    ...
    override fun onMapReady(googleMap: GoogleMap) {
        ...
        try {
            ...
            parent.post {
                ...
                Log.d(TAG, "Map is set")

            }
        } catch (ex: Exception) {
            Log.d(TAG, "REQUIRE MAP VIEW ERROR ${ex.message}")
            ex.printStackTrace()
        }
        ...
        try {
            ...
            DebuggableLogE(TAG, "Can't find style. Error: ", e)
        }
        ...
        * statement mentioned by google: If you are using both NETWORK_PROVIDER and GPS_PROVIDER, then you need to request only the ACCESS_FINE_LOCATION permission, because it includes permission for both providers. Permission for ACCESS_COARSE_LOCATION allows access only to NETWORK_PROVIDER.
        ...
        if (ActivityCompat.checkSelfPermission(
            ...
            try {
                ...
                Log.d(TAG, "Location try location")
...
//                            Log.d(TAG, "Location response gotten")
//
//                            try {
//                                var mLastLocation =
...
//                                        mGoogleApiClient
//                                    )
//                                Log.d(TAG, "Location is $mLastLocation")
//
//                                if (Constants.changedPickupLatlng != null) {
//                                    Log.d(TAG, "Location changing location")
//
//                                    val targetLocation = Location("")
//                                    targetLocation.latitude =
//                                        Constants.changedPickupLatlng!!.latitude
//                                    targetLocation.longitude =
//                                        Constants.changedPickupLatlng!!.longitude
        ...
        geoQuery?.addGeoQueryEventListener(object : GeoQueryEventListener {
            ...
            override fun onKeyEntered(key: String?, location: GeoLocation?) {
                ...
                if (!isTrip) {
                    ...
                    if (iskeyexited && carmarkers.isNotEmpty()) {
...
// carmarker.setIcon(BitmapDescriptorFactory.fromResource(R.drawable.ic_booking_prime_map_topview));
    ...
    override fun onLocationChanged(location: Location) {
        try {
            ...
            if (Constants.changedPickupLatlng != null) {
                ...
                DebuggableLogI(TAG, "Changed Map...")
    ...
    private fun buildGoogleApiClient() {
        ...
        try {
            ...
            Log.d(TAG, "Failed to connecy ${e.message}")
            e.printStackTrace()
        }
    ...
    private fun reqCarTxtChange() {
        if (intent.getStringExtra("Schedule") == "Schedule") {
            ...
            appMainBinding.confirmSeat.text =
    ...
    private fun enableViews(enable: Boolean, resetToRide: Boolean) {
        ...
        if (enable) {
            ...
            if (mLastLocation != null) {
                ...
                Log.d(TAG, "SHOW LOCATION PERMISSION")
    ...
    private fun searchViewset(pickup: LatLng?, drop: LatLng?) {
        if (!isInternetAvailable) {
            ...
            if (pickup != null && drop != null) {
                ...
                locationHashMap = HashMap()
                val pickupLatitude = pickup.latitude.toString()
                val pickupLongitude = pickup.longitude.toString()

                val dropLatitude = drop.latitude.toString()
                val dropLongitude = drop.longitude.toString()
                locationHashMap["pickup_latitude"] = pickupLatitude
                locationHashMap["pickup_longitude"] = pickupLongitude
                locationHashMap["drop_latitude"] = dropLatitude
                locationHashMap["drop_longitude"] = dropLongitude
                locationHashMap["schedule_date"] = sessionManager.scheduleDate.toString()
                locationHashMap["schedule_time"] = sessionManager.presentTime.toString()
                locationHashMap["user_type"] = sessionManager.type.toString()
                locationHashMap["token"] = sessionManager.accessToken.toString()
                ...
                Log.d(
                    "CAR",
                    "WILL SEARCH CAR PickupLat$pickupLatitude PickUpLong $pickupLongitude DROPLAT $dropLatitude DROPLONG $dropLongitude"
                )
                searchCars()
            }
        }
        ...
        when (jsonResp.requestCode) {
            ...
            REQ_SEARCH_CARS -> {
                ...
                if (jsonResp.isSuccess) {
                    Log.d("CAR", "status code is success")

                    if (jsonResp.statusCode == "1") {
                        Log.d("CAR", "status code is 1")
                        commonMethods.hideProgressDialog()
                        onSuccessSearchCar(jsonResp)
                    } else if (jsonResp.statusCode == "2") {
                        Log.d("CAR", "status code is 2")
                        commonMethods.hideProgressDialog()
                    ...
                    Log.d(
                        "CAR",
                        "status code is failed ${jsonResp.statusMsg} and ${jsonResp.errorMsg}"
                    )

                    //This log is print out example result
                    //status code is failed No cars found
...
*/
        ...
        if (!mToolBarNavigationListenerIsRegistered) {
            toggle.toolbarNavigationClickListener = View.OnClickListener {
                ...
                try {
                    ...
                    appMainBinding.cardMap.visibility = View.GONE
                    edit_map.visibility = View.INVISIBLE
                    rltContactAdmin.visibility = View.GONE
    ...
    private fun moveMarkerPolyline(
        ...
        appMainBinding.tvCount.text = count.toString()
        isDownloadUrl = true
  ...
  */
        ...
        if (sessionManager.promoCount > 0) {
            ...
            paymentmethod_type.text = resources.getString(R.string.promo_applied)
            paymentmethod_type.setTextColor(
                ContextCompat.getColor(
                    this,
                    R.color.newtaxi_app_yellow
                )
    ...
    private fun stopTimer() {
        try {
            ...
            if (::timerDirection.isInitialized) {
                timerDirection.cancel()
                timerDirection.purge()
            }
    ...
    fun isTripFunc() {
        ...
        whereto.visibility = View.GONE
        whereto_and_schedule.visibility = View.GONE
        appMainBinding.horiviewsslides.visibility = View.GONE
        appMainBinding.imageslide.visibility = View.GONE
        appMainBinding.cardMapView.visibility = View.GONE
        meetlayout.visibility = View.VISIBLE
        //rltWelcome.visibility = View.GONE
        bottomlayout.visibility = View.VISIBLE
        appMainBinding.cardMap.visibility = View.VISIBLE
        ivFilter.visibility = View.GONE

        //commonMethods.setheaderText(resources.getString(R.string.enrote),common_header)
        //supportActionBar?.show()

        appBarHomeToolBarBinding.tvWelcomeTextSlogan.visibility = View.GONE
    ...
    private fun updateHomeToolBar() {
        ...
        if (!sessionManager.isTrip) {
            ...
            // appMainBinding.tv
            appBarHomeToolBarBinding.tvWelcomeTextSlogan.visibility = View.VISIBLE
            rltWelcome.visibility = View.VISIBLE
        }
