...
import com.newtaxiprime.user.databinding.AppActivitySendingRequestBinding
...
/* ************************************************************
    ...
    *************************************************************** */
class SendingRequestActivity : CommonActivity(), LinearTimer.TimerListener, ServiceListener,
    IFirebaseReqListener, OnMapReadyCallback {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        carname_request = binding.carnameRequest

        progressBar = binding.albumArt

        rl = binding.maplayout

        binding.cfCancel.setOnClickListener{
            cancel()
        }
    ...
    fun circularProgressfunction() {
        ...
        waveDrawable =
            WaveDrawable(ContextCompat.getColor(this, R.color.sending_request_blue), width - 250)

        request_receive_dialog_layout =
            findViewById<View>(R.id.request_receive_dialog_layout) as RelativeLayout
    ...
    fun loadJSONDATA() {
        ...
        try {
            ...
            if (jsonObject.getJSONObject("custom").has("accept_request")) {
                ...
                val trip_id = jsonObject.getJSONObject("custom").getJSONObject("accept_request")
                    .getString("trip_id")
...
//        locationHashMap["pickup_longitude"] = pickup?.longitude.toString()
        val pickupLatLng = LatLng(
            this.locationHashMap?.get("pickup_latitude")!!.toDouble(),
            locationHashMap?.get("pickup_longitude")!!.toDouble()
        )
        ...
        val dropLatLng = LatLng(
            this.locationHashMap?.get("drop_latitude")!!.toDouble(),
            locationHashMap?.get("drop_longitude")!!.toDouble()
        )
